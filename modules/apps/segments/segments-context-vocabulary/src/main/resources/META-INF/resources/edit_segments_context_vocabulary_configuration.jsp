			...
			<div class="sheet sheet-lg">
				<aui:form action="<%= segmentsContextVocabularyConfigurationDisplayContext.getActionURL() %>" method="post" name="fm">
					...
					<div class="form-group">
						<aui:select inlineField="<%= true %>" label="segments-context-vocabulary-configuration-context-name-name" name="contextName" required="<%= true %>" wrapperCssClass="m-0">
							...
							%>
								...
								<aui:option label="<%= option.getLabel(locale) %>" selected="<%= Objects.equals(segmentsContextVocabularyConfigurationDisplayContext.getContextName(), option.getValue()) %>" value="<%= option.getValue() %>" />
						...
						<span class="form-text"><liferay-ui:message key="segments-context-vocabulary-configuration-context-name-description" /></span>
					</div>

					<div class="form-group">
						<aui:select inlineField="<%= true %>" label="segments-context-vocabulary-configuration-vocabulary-name-name" name="vocabularyName" required="<%= true %>" wrapperCssClass="m-0">

							<%
							for (ConfigurationFieldOptionsProvider.Option option : segmentsContextVocabularyConfigurationDisplayContext.getVocabularyNameOptions()) {
							%>

								<aui:option label="<%= option.getLabel(locale) %>" selected="<%= Objects.equals(segmentsContextVocabularyConfigurationDisplayContext.getVocabularyName(), option.getValue()) %>" value="<%= option.getValue() %>" />
